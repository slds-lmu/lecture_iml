...
\end{align*}
   ...
   \vspace*{0.2cm} 
    ...
    \item error term and with that the target outcome given the features are \textbf{normally} distributed, i.e., $\epsilon \sim N(0, \sigma^2) \; \Rightarrow \; (y \vert x) \sim N(x^T \beta, \sigma^2)$\\
...
\end{align*}
    ...
    \begin{itemize}
        ...
        \item Categorical: One-hot-encoding of $L-1$ new features for $L$ categories (dummy encoding). Interpretation (for any of the L-1 categories): The predicted outcome changes for $x_j$ compared to the reference category by $\beta_j$ c.p.
