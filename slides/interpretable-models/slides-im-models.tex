...
\textbf{Update}: Add 2nd order polynomial for \code{temp} \only<2>{(left) and interaction with \code{season} (right)}
\begin{columns}[T, totalwidth=\linewidth]
\begin{column}{0.65\linewidth}
...
\begin{itemize}
    ...
    \item<2>[$\leadsto$] Adding more non-linear and interaction effects makes the model more flexible but also less interpretable
    \item<2>[$\leadsto$] Non-linear and interaction effects need to be specified manually (inconvenient)!
\end{itemize}
