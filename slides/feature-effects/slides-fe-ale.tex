...
\item averaging local effects over conditional distribution $\P(\xv_C|x_S)$ similar to M plots\\ %across all values of $\xv_C$, i.e.,
$\Rightarrow$ Avoids extraploation issue
...
\item Assuming that all interval bounds are close to the corresponding observed values, ALEs are robust to both sources of extrapolation
  \begin{enumerate}
    \item Trained model does not predict in regions that are far away from the training data. %where it was not trained.
