...
%$$\int_{z_{0}}^{x} \E_{\xv_{-S} \vert x_S} \Bigg(  \frac{\partial \fh(x_S, \xv_{-S})}{\partial x_S} \bigg \vert x_S = z_S \Bigg) dz_S - const$$
    ...
    \centerline{$
    \begin{aligned}
      \hspace{2cm} f_{S, PD}(x_S) &= \E_{\xv_{-S}} \left( \fh(x_S, \xv_{-S}) \right) %= \int_{-\infty}^{\infty} \fh(x_S, \xv_{-S}) \, d\P(\xv_{-S})
    \end{aligned}
    $} \\
    \textbf{ALE:}
    \centerline{$
...
%&= \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left(\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right) \textcolor{blue}{dz_S} - const%\\
  &= 
   \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\textcolor{blue}{\xv_{-S} \vert x_S}} \Bigg(  \textcolor{purple}{\frac{\partial \fh(x_S, \xv_{-S})}{\partial x_S}} \bigg \vert x_S = z_S \Bigg) \textcolor{blue}{dz_S - const
...
%&= \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left(\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right) \textcolor{blue}{dz_S} - const%\\
  ...
  \end{aligned}
    ...
    \only<1>{
    \item Difference 1: ALE averages \textcolor{purple}{change of predictions} (via partial derivatives) over \textcolor{blue}{conditional distribution $\P(\xv_{-S}| x_S = z_S)$}
    }
    \only<2-3>{
    \item Difference 1: ALE averages change of predictions (via partial derivatives) over conditional distribution $\P(\xv_{-S}| x_S = z_S)$
    }
    \only<2>{
