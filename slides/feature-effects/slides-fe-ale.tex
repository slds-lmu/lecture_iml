...
%&= \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left(\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right) \textcolor{blue}{dz_S} - const%\\
  ...
  \end{aligned}
    ...
    \only<1>{
    \item Difference 1: ALE averages \textcolor{purple}{change of predictions} (via partial derivatives) over \textcolor{blue}{conditional distribution at $x_S = z_S$}.
    }
    \only<2-3>{
    \item Difference 1: ALE averages change of predictions (via partial derivatives) over conditional distribution at $x_S = z_S$.
    }
    \only<2>{
...
\item For all points within an interval, compute \textbf{prediction difference} when we replace feature value with upper/lower interval bound while keeping other feature values unchanged (blue).
