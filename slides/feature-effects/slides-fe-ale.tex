...
\textbf{But:} May be undesired if interest is to interpret effects within data distribution.
...
\textbf{Results:} M plot of $x_1$ mixes marginal effect of $x_1$ and all other dependent features (here: $x_2$).
}
...
\item averaging local effects over the conditional distribution $\P(\xv_C|x_S)$ similar to M plots.\\ %across all values of $\xv_C$, i.e.,
$\Rightarrow$ Avoids the extraploation issue.
\item accumulating/integrating averaged local effects up to a specific value $x \sim \P(x_S)$.\\ %all values of $z_S$   from $z_0 := \min(x_S)$ u
...
%&= \int_{z_{0}}^{x} \E_{\xv_C \vert x_S} \left(\textcolor{purple}{\frac{\partial \fh(x_S, \xv_C)}{\partial x_S}} \textcolor{blue}{\bigg \vert x_S = z_S} \right) dz_S - const%\\
 &= \int_{z_{0}}^{x} \left( \int_{-\infty}^{\infty}  \textcolor{purple}{\frac{\partial \fh(z_S, \xv_C)}{\partial z_S}} d \textcolor{blue}{\P(\xv_C | z_S) } \,   \right) dz_S - const
}
    \only<2>{\hspace{1cm}
     f_{S, ALE}(x)
%&= \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left(\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right) \textcolor{blue}{dz_S} - const%\\
  &= \textcolor{blue}{\int_{z_{0}}^{x}} \left( \int_{-\infty}^{\infty}\frac{\partial \fh(z_S, \xv_C)}{\partial z_S} d \P(\xv_C | z_S)  \,   \right) \textcolor{blue}{dz_S} - const
}
\only<3>{\hspace{1cm}
     f_{S, ALE}(x)
%&= \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left(\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right) \textcolor{blue}{dz_S} - const%\\
  &= \int_{z_{0}}^{x} \left( \int_{-\infty}^{\infty}\frac{\partial \fh(z_S, \xv_C)}{\partial z_S} d \P(\xv_C | z_S)  \,   \right) dz_S - \textcolor{blue}{const}
}
  \end{aligned}
    $}
    \lz
    \begin{itemize}
    \item<1-> PD directly averages predictions over marginal distribution of $\xv_C$.
    \only<1>{
    \item Difference 1: ALE averages \textcolor{purple}{change of predictions} (via partial derivatives) over \textcolor{blue}{conditional distribution $\P(\xv_C| x_S = z_S)$}.
    }
    \only<2-3>{
    \item Difference 1: ALE averages change of predictions (via partial derivatives) over conditional distribution $\P(\xv_C| x_S = z_S)$.
    }
    \only<2>{
    \item Difference 2: ALE \textcolor{blue}{integrates over $z_S$} partial derivatives of feature $S$ to isolate effect of feature of interest and to remove main effect of dependent features.
    }
    \only<3>{
    \item Difference 2: ALE integrates over $z_S$ partial derivatives of feature $S$ to isolate effect of feature of interest and to remove main effect of dependent features.
    }
...
\begin{itemize}
  \item Partial derivatives not possible for all models (e.g., tree-based methods as random forests).
  \item Partial derivative of prediction function is approximated by computing finite differences of predictions within a set of $K$ intervals across range of observed feature values $\xv_S$:
  $$
  \begin{aligned}
...
\centerline{\includegraphics[width=0.9\textwidth]{figure/ale_interval}}
 ...
 \begin{itemize}
  ...
  \item Finite differences correspond to $\fh(z_{k, S}, \xi_C) - \fh(z_{k-1, S}, \xi_C)$. %and approximates the local effect of the $i$-th observation.
\end{itemize}
...
\centerline{\includegraphics[width=0.9\textwidth]{figure/ale_interval}}
 ...
 \begin{itemize}
  \item Estimate local effect of $\xv_S$ within each interval by averaging all observation-wise finite differences $\hat = $ Approximation of inner integral that integrates over local effects w.r.t. $\P(\xv_C | z_S)$. %conditional distribution of $\xv_C$ on $\xv_S$.
