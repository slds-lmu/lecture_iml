...
LM without interaction: Coefficient $\hat\beta_j$ (constant for all observations) is effect of feature $x_j$:
...
Sample grid values $x_S^*$ along feature of interest $x_S$ (here, grid is $x_S^* = x_1^* \in \{1, 2, 3\}$) and replace original feature value of $i$-th observation with these sampled grid values.
%$\Rightarrow$ New artificial data points for $i$-th observation.
\newline $\Rightarrow$ We obtain new artificial data points for the $i$-th observation.
...
\end{column}
\begin{column}{0.55\textwidth}
\includegraphics[page=1, width=0.85\textwidth]{figure/ICE}
\end{column}
\end{columns}
\vspace*{\topsep}
...
\begin{itemize}
\item equidistant values within range of considered feature,
