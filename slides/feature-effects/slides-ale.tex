...
\textbf{Question:} Can we avoid the extrapolation issue by averaging only over points in the neighborhood of a specific point $x_1$? %, i.e., using the conditional distribution at
}
...
$\Rightarrow$ M plots are useless to assess the main effect of a feature in case of dependent features.
...
\begin{center}
\includegraphics[width=0.7\textwidth]{figure_man/pd_vs_mplot}
\end{center}
...
%The constant $c_1$ will be chosen so that the ALE plot is centered vertically.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{itemize}
...
\frame{
  \frametitle{PD vs. ALE}
    ...
    $
    \begin{aligned}
      \hspace{2cm} \E_{\xv_C} \left( \fh(x_S, \xv_C) \right) = \int_{-\infty}^{\infty} \fh(x_S, \xv_C) \, d\P(\xv_C)
    \end{aligned}
    $ \\
    \textbf{ALE:}
    $
    \begin{aligned}
      \only<1>{\hspace{1cm}
      \int_{z_{0}}^{x} \E_{\xv_C \vert x_S} \left[\textcolor{purple}{\frac{\partial \fh(x_S, \xv_C)}{\partial x_S}} \textcolor{blue}{\bigg \vert x_S = z_S} \right] dz_S\\
        = \int_{z_{0}}^{x} \left[ \int_{-\infty}^{\infty}  \textcolor{purple}{\frac{\partial \fh(x_S, \xv_C)}{\partial x_S}} d \textcolor{blue}{\P(\xv_C | z_S) } \,   \right] dz_S}
    \only<2>{\hspace{1cm}
    \textcolor{blue}{\int_{z_{0}}^{x}} \E_{\xv_C \vert x_S} \left[\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} \bigg \vert x_S = z_S \right] \textcolor{blue}{dz_S}\\
  = \textcolor{blue}{\int_{z_{0}}^{x}} \left[ \int_{-\infty}^{\infty}\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} d \P(\xv_C | z_S)  \,   \right]}
  \end{aligned}
    $
    \lz
    \begin{itemize}

    \only<1>{\item Difference 1: PD averages predictions over the marginal distribution while ALE averages the \textcolor{purple}{change} of predictions (defined as the gradient) over the \textcolor{blue}{conditional} distribution.}
    \only<2>{\item Difference 2: ALE has an additional \textcolor{blue}{integral over $z_S$}. Instead of directly averaging the predictions, the ALE method calculates the prediction differences conditional on features S and integrates the derivative over features $S$ to estimate the effect. The derivative (or interval difference) isolates the effect of the feature of interest and blocks the effect of correlated features.}
    \end{itemize}
}
