...
Let $z_0$ denote the minimum of observed values of feature $\xv_S$, i.e., $z_0 = \min(\xv_S)$. All complementary features are denoted by $\xv_C$. ALE plots are based on
\begin{enumerate}
...
$\Rightarrow$ Avoids the OVB issue.
\end{enumerate}
...
\frame{
  ...
  = \textcolor{blue}{\int_{z_{0}}^{x}} \left[ \int_{-\infty}^{\infty}\frac{\partial \fh(x_S, \xv_C)}{\partial x_S} d \P(\xv_C | z_S)  \,   \right] \textcolor{blue}{dz_S}}
  \end{aligned}
    $}
    \lz
    \begin{itemize}

    \only<1>{\item Difference 1: PD averages predictions over the marginal distribution while ALE averages the \textcolor{purple}{change} of predictions (defined as the partial derivative) over the \textcolor{blue}{conditional} distribution.}
