...
\textbf{Definition:} PD function is expectation of $\fh(\xv_S, \xv_C)$ w.r.t. marginal distribution of features $\xv_C$:
$$f_{S, PD}(\xv_S) = \E_{\xv_C} \left( \fh(\xv_S, \xv_C) \right) = \int_{-\infty}^{\infty} \fh(\xv_S, \xv_C) \, d\P(\xv_C)$$

\textbf{Estimation:} For a grid value $\xv_S^*$, average ICE curves point-wise at $\xv_S^*$ over all observed $\xv_C^{(i)}$:
$$\fh_{S, PD}(\xv_S^*) = \frac{1}{n} \sum_{i=1}^n \fh(\xv_S^*, \xv_C^{(i)})$$
...
\begin{itemize}
 \item Humidity and temperature interact with each other at high values (see shape difference on top-right)\\
