...
\begin{itemize}
\item
  \textbf{ICE:} Visualizes how \textbf{prediction of
  individual observation} changes if feature values vary\\
  \(\Rightarrow\) ICE is \textbf{local} interpretation method (black curves).
\item
  \textbf{PD:} Visualizes average effect of a feature,
  i.e., how \textbf{expected prediction} changes if feature values vary
  \(\Rightarrow\) PD plot is \textbf{global} interpretation method (yellow curve).
\end{itemize}
}
...
\begin{itemize}
  ...
  \item Parallel ICE curves across all observations indicate homogeneous effect.
...
\begin{itemize}
\item PD plot visualizes the expected prediction if all observations would belong to a certain category (and shows distribution of predictions in a box plot).
\item ICE plot connects predictions of individual observations (might identify heterogenous effects between categories if many lines are crossing).\\
%to identify heterogenous effects between categories (here: lines between SPRING and SUMMER are heterogenous).\\
\textbf{Note:} For nominal categories without ordering, categories could be compared pairwise.
\end{itemize}
