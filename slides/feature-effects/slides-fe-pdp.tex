...
Estimate PD function by \textbf{point-wise} average of ICE curves at grid value \fcolorbox{red}{white}{$x_S^* = x_1^* = \only<1>{1}\only<2>{2}\only<3>{3}$}:
...
\begin{itemize}
 ...
 $\leadsto$ Shape of ICE curves at different horizontal and vertical slices vary (if both values are high)
