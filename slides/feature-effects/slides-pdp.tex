...
\newcommand{\titlefigure}{figure_man/pdp_num}
...
The partial dependence (PD) plot is the expectation of ICE curves w.r.t. the marginal distribution of complementary features $\xv_C$,
$$f_{S, PD}(x_S) = \E_{\xv_C} \left( \fh(x_S, \xv_C) \right) = \int_{-\infty}^{\infty} \fh(x_S, \xv_C) \, d\mathcal{P}(\xv_C)$$

For a grid value $x_S^*$, the PD function is estimated by the point-wise average of the ICE curves:
...
\begin{center}
\includegraphics[width=0.7\textwidth]{figure_man/pdp_num}
\end{center}
...
\begin{center}
\includegraphics[width=0.9\textwidth]{figure_man/pdp_ice_cat}
\end{center}
