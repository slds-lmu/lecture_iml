...
% Plotting ALE
% \begin{frame}{Plotting the ALE Curve}

% \begin{enumerate}
%   \item \textbf{Midpoints}  
%         For each interval $[z_{k-1,S},z_{k,S}]$ compute  
%         $m_k = \dfrac{z_{k-1,S}+z_{k,S}}{2}$.
%   \item \textbf{Uncentered ALE at midpoints}  
%         $\displaystyle \hat{\tilde{f}}_{S,\text{ALE}}(m_k)$
%         (definition as on previous slide).
%   \item \textbf{Centering}  
%         $\displaystyle \hat{f}_{S,\text{ALE}}(m_k)
%         \;=\; \hat{\tilde{f}}_{S,\text{ALE}}(m_k)
%         \;-\;\frac{1}{n}\sum_{i=1}^{n}
%               \hat{\tilde{f}}_{S,\text{ALE}}\!\bigl(x_S^{(i)}\bigr)$.
%   \item \textbf{Plot} the pairs $(m_k,\,\hat{f}_{S,\text{ALE}}(m_k))$ and connect successive points
%         with straight lines.
% \end{enumerate}

% \medskip
% \textbf{Plot smoothness}: \\
% The model is evaluated at interval end-points $\Rightarrow$ total number of evaluations ($2n$) is independent of the number of intervals ($K$).
% \\
% $\leadsto$ One can pick a larger $K$ to produce a smoother curve, provided each
% interval still contains enough data to be representative.
% \end{frame}
