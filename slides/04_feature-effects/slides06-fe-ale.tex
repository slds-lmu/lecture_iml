...
\item \textbf{Accumulate:} Integrate averaged local effects up to a specific value $x \sim \P(x_S)$\\ %all values of $z_S$   from $z_0 := \min(x_S)$ u
...
= \underbrace{\int_{z_0}^{x}}_{\text{acc.}} 
\underbrace{\E_{\xv_{-S} \mid x_S = z_S}}_{\text{average locally}} 
\left( \underbrace{\frac{\partial \hat{f}(z_S, \xv_{-S})}{\partial z_S}}_{\text{local effect}} \right) dz_S
...
\begin{frame}{ALE Estimation: 2-D Illustration}
...
\begin{itemize}
\tightlist
\item Partition the feature range of $x_S$ into $K$ intervals (vertical lines)
\item For each data point in interval $k$, i.e., $x_S^{(i)} \in \; [z_{k-1, S}, z_{k, S}]$:
  \begin{itemize}
    \item Replace $x_S^{(i)}$ with \textcolor{blue}{interval bounds ($z_{k-1, S}$ and $z_{k,S}$)}, keeping $\xv_{-S}^{(i)}$ fixed
    \item Compute \textbf{prediction difference} (local effect) for $i$-th data point \\
    $\leadsto$ Finite difference: $\fh(z_{k, S}, \xi_{-S}) - \fh(z_{k-1, S}, \xi_{-S})$%\\
    %$\leadsto$ Approximates local effect of $x_S$ for $i$-th data point
  \end{itemize}
\item Average these finite differences over all \( i \) in each interval \\
$\leadsto$ Approximates the \textbf{inner integral} \( \E_{\xv_{-S} \mid x_S = z_S} \left[ \partial \hat{f}/\partial z_S \right] \)

\item Accumulate these averages from \( z_0 \) to the point of interest \( x \in \mathcal{X}_S \) \\
$\leadsto$ Approximates the \textbf{outer integral} over \( z_S \in [z_{0}, x] \) to yield the ALE function
