...
% Note: Same idea called SFIMP was introduced in \citebutton{Casalicchio et al. (2020)}{https://arxiv.org/abs/1804.06620}
...
\begin{itemize}
  ...
  \begin{itemize}
      \item e.g., if $x_1$ and $x_2$ are perfectly correlated, even if only $x_1$ has nonzero coefficients, both are considered equally important
  \end{itemize}
...
%\end{column}
%\begin{column}{0.5\textwidth}
Conditional $v(j)$ are nonzero for features that are not used by $\fh$

Marginal $v(j)$ are only nonzero for features that are used by $\fh$
...
\begin{align*}
    \phi_j = \frac{1}{M} \sum_{m=1}^M v(j \cup S(j;\pi_m)) - v(S(j;\pi_m))
\end{align*}
