...
\newcommand{\learninggoals}{
	\item Understand Conditional Feature Importance
...
$\Rightarrow$ Since PFI evaluates the model on unrealistic observations, the features $x_1$ and $x_2$ are nevertheless considered relevant
 ...
 Let $\pert{x}{S}{-S}$ be the feature vector where features $x_S$ were replaced with a sample from $\P(x_S|x_{-S})$. Then
%
\begin{align*}
...
\begin{itemize}
  \item Measure the error \color{red}\textbf{with permuted feature values} \color{black} $\pert{x}{S}{-S}$, where $\pert{x}{S}{-S}_S \sim \P(x_S|x_{-S})$
