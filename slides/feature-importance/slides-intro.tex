...
Feature importance methods provide highly condensed insight, but can only highlight certain aspects of model and data. For example, one may be interested in one of the following questions:
\lz
\begin{enumerate}
...
\begin{vbframe}{Terminology}

Explain what I mean on the previous slide with the formulations.
%TODO
\end{vbframe}

\begin{vbframe}{Potential Interpretation Goals}

The properties do not necessarily coincide. Can be proven by example (sketch):

\begin{itemize}
  \item A feature may causal for the prediction $\yh$ (1) without containing prediction-relevant information about $\ydat$ (2). \textit{Example:} overfitting.
  \item A feature may contain prediction relevant information (2) without causing the prediction (1).  \textit{Example:} underfitting.
  ...
  \textit{Example:} random forest model on dataset with $y \not \ind x_1$ and $y \ind x_2 | x_1$ where $x_1$ was not drawn as potential split feature for one or more trees, such that the respective trees split on $x_2$ instead.
\end{itemize}
$\Rightarrow$ In general, an importance score cannot serve all goals at once.
\end{vbframe}
