...
\begin{vbframe}{Permutation Feature Importance: Extrapolation}
 ...
 \textbf{Example:} Let $x_1 :=  \epsilon_1$, $x_2 := x_1 + \epsilon_2$, $x_3 := \epsilon_3$, $x_4 := \epsilon_4$, $y = x_3 + \epsilon_y$ with independent noise terms $\epsilon_1, \epsilon_3, \epsilon_4 \sim N(0,1)$, $\epsilon_2 \sim N(0, 0.01)$, $\epsilon_Y \sim N(0, 0.1)$.
A lm is fit, yielding $\fh(x) \approx 0.3 x_1 - 0.3 x_2 + x_3$.\\
 %
\begin{figure}
\hfill
...
\begin{enumerate}
    \item Is the feature $x_j$ causal for the prediction?
    \begin{itemize}
    ...
    \end{itemize}
    \item Does the variable $x_j$ contain prediction-relevant information about $y$ (that the model exploits)?
    \begin{itemize}
    ...
    \end{itemize}
    \item Does the model require access to $x_j$ to achieve it's prediction performance? (I.e. does the feature contribute unique information about $y$?)
    \begin{itemize}
...
\begin{vbframe}
  Apply PIMP to extrapolation example?
\end{vbframe}
