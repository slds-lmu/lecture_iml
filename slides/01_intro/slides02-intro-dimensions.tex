...
\begin{frame}{Levels of interpretability}
	\begin{center}
        ...
        \only<1>{1\textsuperscript{st} level view && \multicolumn{3}{c}{How does one particular model given one data set operate?} \leavevmode\\ &&&\\}

        \only<3>{2\textsuperscript{nd} level view && \multicolumn{3}{c}{\makecell{How was a model chosen based on a data set? \\ (E.g., hyperparameter tuning, model class selection)}} \leavevmode\\ &&&\\}

        \only<5>{3\textsuperscript{rd} level view && \multicolumn{3}{c}{\makecell{How do the properties of data sets (e.g., number of observations/variables/ \\missing values) relate to the particular model selection?}} \leavevmode\\} &&&\\
    \end{tabular}
\end{center}
\end{frame}
