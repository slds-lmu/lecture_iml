...
Examples of critical areas where decisions based on ML models can affect human life 
	...
	\begin{column}{0.45\textwidth}
        \includegraphics[page=1, width=\textwidth, trim = 0 250 0 0]{figure/counterfactual.pdf}
        \medskip
...
\begin{itemize}
    \item Year (\code{yr}) and Temperature (temp) are the most important features
    \item Holiday (\code{holiday}) is less important (Can we drop it?)
\end{itemize}
...
\begin{frame}{Explain to justify: prediction on subgroups an fairness predictions}
