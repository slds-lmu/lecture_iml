...
% Use of $$...$$ \rho(X_1, X_2) equation leads to content overflow at bottom of the slide, therefore \centerline + $...$
\begin{frame}[t]{Pearson's correlation coefficient $\rho$}
\textbf{Correlation} often refers to Pearson's correlation (captures only \textbf{linear relationship})\\
$$\rho(X_1, X_2) = \tfrac{\sum_{i=1}^{n}(x_1^{(i)}-\bar{x}_1)(x_2^{(i)}-\bar{x}_2)}{\sqrt{\sum_{i=1}^{n}(x_1^{(i)}-\bar{x}_1)^2 \sum_{i=1}^{n}(x_2^{(i)}-\bar{x}_2)^2}} \in [-1, 1]$$
\spacer[0.125]
\splitVTT[0.55]{
\spacer[0]
\image{figure/pearson_cor}
}{
...
\item $\rho = 0$ if area of rectangles cancels out$\leadsto$ $X_1$, $X_2$ linearly uncorrelated
\end{itemizeM}
%\textbf{But:} Pearson's $\rho$ does not measure non-linear dependencies, e.g., $\rho = 0$ but dependent:
\end{frame}


\begin{frame}[t]{Coefficient of determination $R^2$}
Another method to evaluate \textbf{linear dependency} between features is $R^2$
\spacer[1]
\splitVTT[0.48]{
\spacer[0]
\only<1>{\image{figure/r_squared_Fahrenheit}}
\only<2>{\image{figure/r_squared_compare}}
\only<3>{\image{figure/r_squared}}
}{
\begin{itemizeM}
...
\begin{frame}[t]{Joint, Marginal and Conditional Distribution}
...
\begin{align*}
p_{X_1|X_2}(x_1 \mid x_2) &= \P(X_1 = x_1 \mid X_2 = x_2) \\
...
& $x_2 = 0$ & $x_2 = 1$ \\
\hline
$\P(X_1 = 0 \mid X_2 = x_2)$ & \cellcolor{gray}0.67 & \cellcolor{gray}0.43 \\
\hline
$\P(X_1 = 1 \mid X_2 = x_2)$ & \cellcolor{gray}0.33 & \cellcolor{gray}0.57 \\
\hline
$\sum$ & 1 & 1 \\
\hline
\end{tabular}
...
\begin{frame}[t]{Mutual information: example}
...
\begin{frame}[t]{Dependence and Independence}
\textbf{Example:}
\splitVTT[0.49]{
\spacer[0]
\image{figure/independent_slice.pdf}
...
\pause
\spacer[0]
\image{figure/dependent_slice.pdf}
Conditional distributions do not match their marginal distributions
}
\end{frame}


% Use of minipage in order to center-align "\rho" descriptors with the corresponding plots
\begin{frame}[t]{Correlation vs. Dependence}
...
\end{minipage}
\end{center}
\imageC[0.9]{figure/dnorm_correlation.pdf}
}
\only<2>{
\begin{center}
