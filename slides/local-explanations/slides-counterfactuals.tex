...
\begin{vbframe}{Mathematical Perspective}
	...
	\begin{itemize}
		...
		\item $y' \in \R^g$ as desired outcome 
	\end{itemize}
	\vspace{0.3cm}
	...
	\begin{itemize}
		...
		\framebreak
		\item $o_1$ could be, for example, L1-distance $o_1(\fh(\xv'), y') = |\fh(\xv')-y'|$ for regression or the indicator function $o_1(\fh(\xv'), y') = \Ind_{\{ \fh(\xv') \neq y' \}}$ for binary classification.
		...
		The value of $d_G$ depends on the feature type:
		\begin{equation*}
		d_G(x_j, x^*_j) = 
		\begin{cases}
