...
\pause
	...
	\begin{itemize}[<+->]
		...
		\item The above statement is true, if in all possible worlds most similar to the actual world where $S$ had been the case, $Q$ would have been the case
...
%``$Q$ causally depends on $S$ iff, if $S$ were not the case $Q$ would not have been the case.''
	...
	\item Counterfactuals have largely been studied to give an account of causal dependence
		...
		$\leadsto$ e.g., decreasing loan amount by \$20.000 and being one year older is recommended by the explainer but only loan amount would be causally relevant
...
$$\argmin_{\xv'} \lambda_1 o_1(\fh(\xv'), y') + \lambda_2 o_2(\xv', \xv)$$
	...
	\begin{itemize}
		...
		\item Choice of $o_1$ (distance on prediction space) and of $o_2$ (distance on the feature space) is crucial
	\end{itemize}
\end{frame}
...
%However, to end-users, these explanations appear to explain the process in which the model is employed. 

	\begin{itemize}[<+->]
	\item Most CEs provide explanations of model-predictions (not the real-world), but CEs might appear to explain the real-world for end-users\\
