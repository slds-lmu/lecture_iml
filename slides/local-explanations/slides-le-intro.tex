...
\begin{columns}
	...
	\begin{column}{0.5\textwidth}
    	...
    	\begin{itemize}
    	    ...
    	    \item By using local explanation methods, the bank could e.g. provide a counterfactual explanation:
    	    \begin{itemize}
    	        \item[] ``If you were older than 21, your loan application would have been accepted."
    	    \end{itemize}
    	    \item[$\leadsto$] helps to understand the decision and to take actions for recourse (if req.)
    	\end{itemize}
...
\begin{columns}
	...
	\begin{itemize} 
	    \item Imagine: 
	        \begin{itemize}
	            \item You are working in a car company
...
\begin{vbframe}[c]{Characteristics}
	\begin{itemize}[<+->]
		...
		\item \textbf{Model classes:} Mostly model-agnostic by definition, but model-specific for computational reasons $\leadsto$ very popular also for deep learning models
...
\begin{vbframe}{Credit Dataset}
	...
	\begin{itemize}
		...
		\item It has 522 complete observations and nine features containing credit and customer information
		\item Binary target indicates whether a customer has a `good' or `bad' credit risk  
