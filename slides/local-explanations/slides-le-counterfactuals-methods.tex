...
\begin{itemize}
	...
	\centering
			%\begin{center}
		\includegraphics[width=0.65\textwidth]{figure/counterfactuals_credit_parallel}\\
			%\end{center}
		%\vspace{-0.2cm}
			\scriptsize{\textbf{Figure:} Parallel plot: Grey lines show feature values of CEs $\xv'$, blue line are values of $\xv$. Features without proposed changes are omitted. Bold numbers refer to range of numeric features.} 
		...
		\begin{column}{0.5\textwidth}
			%\begin{center}
			\centering
			\includegraphics[width=0.9\textwidth]{figure/counterfactuals_credit_heat}\\
			%\end{center}
		%\vspace{-0.2cm}
			\scriptsize{\textbf{Figure:} Response surface plot. White dot is $\xv$, black dots are CEs $\xv'$. Histograms show marginal distribution of training data $\Xmat$.} 
		\end{column}
		}
	\end{columns}
...
\begin{itemize}[<+->]
    ...
    CEs explain ML decisions by pointing to few specific alternatives which reduces complexity, but is limited in explanatory power $\leadsto$ psychologists showed that even though the perceived model-understanding of end-users increases, the objective model-understanding remains unchanged
...
\begin{itemize}[<+->]
    ...
    \item \textbf{Actionability vs. fairness:} Some authors suggest to focus only on the actionability of CEs $\leadsto$ this counteract functions like contestability, e.g., if ethnicity is not permuted in a CE since it is not actionable, this could lead to hiding racial biases in the model
    \item \textbf{Attacking CEs:} Researchers can create models with great performance, which generate arbitrary explanations specified by the ML developer $\leadsto$ how faithful are CEs to the models underlying mechanism?
    \item \textbf{Assumption of constant model:} To provide guidance for the future, CEs assume that their underlying model would not change in the future $\leadsto$ in reality this assumption is often violated and CEs are not reliable anymore 
\end{itemize}
