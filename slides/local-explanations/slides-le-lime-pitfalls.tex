...
\begin{vbframe}[c]{Pitfall: Sampling}
	\begin{itemize}
	\itemsep1em
    ...
    %  	\end{center}
     ...
     \begin{columns}[T, totalwidth=\linewidth]
        ...
        \includegraphics[width=\textwidth, trim = 0px 0px 30px 40px, clip]{figure/lime_locality}
         ...
         \begin{itemize}
             \item Surrogate models for 2 obs. (green) for same model
...
\begin{vbframe}[c]{Pitfall: Local vs. Global Features \citebutton{Laugel et al. 2018}{https://arxiv.org/pdf/1806.07498.pdf}}

\begin{itemize}%[<+->]
	\item<1-> \textbf{Problem}: By sampling instances for the surrogate model from the whole input space, the influence of local features is hidden in favor of features with global influence (even if kernel width is reduced)
...
\begin{vbframe}{Pitfall: Local vs. Global Features -- Example \citebutton{Laugel et al. 2018}{https://arxiv.org/pdf/1806.07498.pdf}}
...
\begin{columns}
	\begin{column}{0.6\textwidth}
		...
		\end{itemize}
		\item \textbf{Observation:} Decision boundaries of LIME with different kernels (blue and green lines) do not match the direction of the local decision boundary\\ (which may appears steeper)
	\end{itemize}
\end{column}
...
\begin{vbframe}{Pitfall: Local vs. Global Features -- Solution \citebutton{Laugel et al. 2018}{https://arxiv.org/pdf/1806.07498.pdf}}
...
\begin{vbframe}[c]{Pitfall: Faithfulness}
\begin{itemize}
%\itemsep1em
...
\begin{vbframe}{Pitfall: Hiding biases \citebutton{Slack et al. 2020}{https://arxiv.org/abs/1911.02508}}

\begin{itemize}
	\item \textbf{Problem}: Developer could manipulate their model to hide biases 
	\item \textbf{Observation}: LIME can sample out-of-distribution points
	\pause
	\item \textbf{Attack}:
	\begin{enumerate}
	...
	\end{enumerate}
	\item \textbf{Example}: Credit dataset\\
	 $\leadsto$ biased model trained on feature `gender`\\
	 $\leadsto$ unbiased model could be trained on features uncorrelated with `gender`
\end{itemize}
\end{vbframe}

\begin{vbframe}[c]{Pitfall: Robustness \citebutton{Alvarez-Melis, D., \& Jaakkola, T. 2018}{https://arxiv.org/abs/1806.08049}}
\begin{itemize}
...
\begin{vbframe}{Pitfall: Definition of Superpixels \citebutton{Achanta et al. 2012}{https://ieeexplore.ieee.org/document/6205760}}
