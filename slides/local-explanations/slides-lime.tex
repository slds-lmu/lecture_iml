...
\begin{vbframe}{Formal Definition}
	LIME provides a local explanation for a black-box model $\fh$ in form of a model $g \in \Gspace$ with $\Gspace$ as the class of potential (interpretable) models. This model $g$ should have two characteristics:
	\begin{enumerate}
		\item It should be \textbf{interpretable}, i.e. relation between the input variables and the response are easy to understand.  
		\item It should be \textbf{locally faithful}, i.e. it should behave similarly to $\fh$ in the vicinity of the instance being predicted. This characteristic is also called local fidelity. 
	\end{enumerate}
...
\vspace{0.5cm}
 	...
 	\begin{vbframe}{Local model fidelity}
 		\begin{itemize}
 			\item A model $g$ is locally faithful to $\fh$ w.r.t. a point $\xv$ if for points $\zv \in \Zspace \subseteq \R^p$ close to $\xv$ the predictions of $g(\zv)$ are close to $\fh(\zv)$. 
 			...
 			\begin{enumerate}
 				...
 				with $\sigma$ as the kernel width. $d$ could be for example the Euclidean distance (numeric features) or the Gower distance $d_G$ (mixed features). 
...
\begin{vbframe}{Minimization task}
	\begin{itemize}
		...
		\item The model complexity ($J(g)$) is determined by users beforehand by restricting the class $\Gspace$. For example, users could only consider sparse linear models. 
