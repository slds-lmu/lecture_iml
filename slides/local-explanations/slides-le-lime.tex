...
\begin{frame}{Model Complexity}
    ...
    We can measure the complexity of a model $g$ using a complexity measure $J(g)$ \lz
 	...
 	\begin{itemize}
 	    \item Let $\Gspace = \left\{g:\Xspace \to \R ~|~g(\xv) = \sum_{m=1}^M c_m \Ind_{\{ \xv \in Q_m \}}\right\}$ be the class of trees\\
 	     i.e., the class of additive (constant $c_m$) models over the leaf-rectangles $Q_m$
...
\begin{frame}{Local model fidelity}
 	\begin{itemize}
 		...
 		\item In an optimization task: the closer $\zv$ is to $\xv$, the closer $g(\zv)$ should be to $\fh(\xv)$  
 		\pause
