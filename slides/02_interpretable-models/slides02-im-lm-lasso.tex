...
$$y = \theta_0 + \theta_1 x_1 + \theta_2 x_2 + \dots + \theta_p x_p + \epsilon = \xv^\top \theta + \epsilon$$
    ...
    \begin{column}{0.6\textwidth}
    \begin{itemize}
        \item \textbf{p-value}: probability of obtaining a test statistic that is more extreme (values that speak against $H_0$) as the test statistic computed from the sample, assuming $H_0$ is correct.
...
\medskip
\centering
\includegraphics[width = 0.5\textwidth]{figure/reg_lm_plot_interpreted.pdf}
