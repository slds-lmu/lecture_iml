...
\begin{itemizeS}
\item For predicted probabilities: $o_{target}(\fh(\xv'), y') = |\fh(\xv') - y'|$ 
\vspace{0.1cm}
\item For predicted hard labels: $o_{target}(\fh(\xv'), y') = \mathds{1}_{\{\fh(\xv') \ne y'\}}$
\end{itemizeS}
\end{itemizeM}
\pause
\spacer[0.25]
...
% \mathds{1}_{\left\{ x'_j \ne x_j \right\}} & \text{if $x_j$ is categorical}
% \end{cases}
% \end{equation*}
\begin{itemizeM}
\item $\delta_G(x'_j, x_j) = \mathds{1}_{\{x'_j \ne x_j\}}$ if $x_j$ is categorical
\item $\delta_G(x'_j, x_j) = \dfrac{1}{\widehat{R}_j} |x'_j - x_j|$ if $x_j$ is numerical\\
...
$$
o_{sparse}(\xv', \xv) = \sum_{j = 1}^p \mathds{1}_{\{x'_j \neq x_j\}}
$$
\end{itemizeM}
\end{framev}
