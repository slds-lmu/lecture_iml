...
$h_x\left(\mathbf{z}^{\prime (k)}\right)=\mathbf{z}^{(k)} \text { where } h_x:\{0,1\}^{p} \rightarrow \mathbb{R}^{p}$
 maps 1’s to feature values of instance $\xv$ for features part of the $k$-th coalition and 0's to feature values of a \color{orange}{randomly sampled instance} \color{black}for features absent in the $k$-th coalition
