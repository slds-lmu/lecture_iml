...
\end{itemizeS}%Convention: $v(\emptyset) = 0$ and total payout  $v(P)$ to be fairly distributed among players.
...
\begin{framev}[align=top]{Cooperative Games - No Interactions}
...
$\leadsto$ Total value $v(P) = 6000$ splits proportionally by individual contribs:
\centerline{$
...
\visible<1>{\centering \textbf{Question:} What are individual marginal contributions and what's a fair payout?}% if contributions are $\tfrac{1}{2}$ (blue), $\tfrac{1}{3}$ (yellow), $\tfrac{1}{6}$ (red)?}
...
\splitVTT[0.459]{
\spacer[0]
\includegraphics[page=3,width=0.9\textwidth,trim=0px 75px 390px 0px,clip]{figure/Shapley.pdf}%
\spacer[0]
}{
\spacer[0]
\begin{footnotesize}
...
\item Averaging over subsets does not recover total payout $v(P)$\\
$\leadsto$ unfair payout distribution\\
$\leadsto$ avg. contrib. \colorcircle{playerred}{white}{1} = 1750 \colorcircle{playeryellow}{white}{2} = 1750 \colorcircle{playerblue}{white}{3} = 2250 does not sum to $v(P) = 6000$
...
\spacer[0]
%\begin{center}
\includegraphics[page=3,width=0.9\textwidth,trim=0px 50px 420px 50px,clip]{figure/Shapley.pdf}
%\end{center}
\spacer[0]
}{
...
\begin{framev}[align=top]{Shapley Values - Illustration}
\begin{itemizeM}
...
\only<1>{\item[] \phantom{\textbf{Example:} Compute payout diff. after player $1$ enters coalition $\leadsto$ average}}%
\only<2>{\item \textbf{Example:} Compute payout diff. after player $1$ enters coalition $\leadsto$ average}%
\only<3>{\item \textbf{Example:} Compute payout diff. after player $2$ enters coalition $\leadsto$ average}%
\only<4>{\item \textbf{Example:} Compute payout diff. after player $3$ enters coalition $\leadsto$ average}%
\end{itemizeM}
\begin{center}
...
\item Each $S$ occurs in $|S|!(|P|-|S|-1)!$ orderings $\leadsto$ Weight: $\frac{|S|!(|P| - |S| - 1)!}{|P|!}$
\end{itemizeM}
\end{framev}


\begin{framev}[align=top]{Shapley Value - Set definition}
...
\splitVTT[0.44]{
\spacer[0]
\image{figure_man/multinom_coef_over_size.png}
}{
\spacer[0.6]
\begin{itemizeM}
\item $|S| = 0$: player $j$ joins first\\
