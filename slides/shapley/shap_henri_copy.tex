...
\vspace{1cm}
\textbf{Problem}\\
How do we estimate the Shapley values $\phi_i$?
\end{onlyenv}
...
\begin{itemize}
    ...
    \begin{onlyenv}<2>
    \item S is empty -
    use the weighted average of predictions of all terminal nodes. \\
...
\centering
    ...
    \path (A) ++(7:6cm) node [node, label=below:{n=5}] (B) {count = 3200};
    \path (A) ++(-7:6cm) node [node, label=below:{n=5}] (C) {temp $>$ 30};
    \path (C) ++(7:6cm) node [node, label=below:{n=3}] (D) {count = 6850};
    \path (C) ++(-7:6cm) node [node, label=below:{n=2}] (E) {count = 5400};
...
\begin{onlyenv}<1>
$$
S = \varnothing \rightarrow \phi_0 = \frac{3200*5 + 6850*3 +5400*2}{10}= 3775
$$
\end{onlyenv}
