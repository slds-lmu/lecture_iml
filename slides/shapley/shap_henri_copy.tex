...
\begin{itemize}
    \item We define a simplified (binary) coalition feature space $\mathbf{Z}^\prime$ with $K$ rows and $p$ columns 
    \item $\mathbf{z}^{\prime (k)} = \{z^{\prime (k)}_1, \ldots, z^{\prime (k)}_p\}$ with $k \in \{1,\ldots, K\}$ indexes the $k$-th coalition 
\end{itemize}
}
\vspace{0.5cm}
\begin{exampleblock}{}
...
\draw<1-4>[arrow]
  (zex.east) to[out=180,in=90] ([xshift= 1ex, yshift=1.4ex]{pic cs:zp});  
\draw<2-3>[arrow]
...
\begin{itemize}
   ...
   \item $\mathbf{z}^{\prime (k)}$: 1 if features are in coalition $S$, 0 otherwise
...
$h_x\left(\mathbf{z}^{\prime (k)}\right)=\mathbf{z}^{(k)} \text { where } h_x:\{0,1\}^{p} \rightarrow \mathbb{R}^{p}$
 which maps 1’s to feature values of instance $\xv$ for features in $S$ and 0's to feature values of a \color{orange}{randomly sampled instance} \color{black}for features in $C = S^\complement$
