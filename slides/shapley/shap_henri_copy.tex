...
\begin{onlyenv}<1>
$h_{x}$ represent a function that maps 1’s to the corresponding value from the instance x that we want to explain:. $h_{x}$ connects our coalition vector to the underlying data.
\end{onlyenv}
...
\node<3-> [right=of tab1] (tab2) {%
  ...
  \end{tabular}};
\draw<2->[->]
(tab2.north) to[out=150,in=30] node[below]{$h(\cdot)$} (tab1.north) ;
\node<2-> [below=of tab1] (eq) 
  {$\begin{array}{lllllcr}
\hat{y} & = & \phi_0 &+ \phi_{temp} &+ \phi_{hum} &+ \phi_{windspeed} &+ \phi_{yr}  \\
6825 & = & 4469 &+ 1809 &+ 450 &+ 241 &- 144 
\end{array}$};
