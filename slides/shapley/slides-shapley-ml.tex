...
\begin{itemize}
  \item Players $\hat{=}$ feature values of i-the observation $x^{(i)}_j, j \in \pset$.
  \item Features collaborate to make a prediction $\fh(\xi)$.
...
\item The marginal prediction is $\fh_{S}(\xv_S) = \int_{X_C} \fh(\xv_S, X_C)d \P_{X_C}$
 ...
 \item By using the marginal prediction, we have defined what it means for features to be \enquote{missing} for the prediction: We remove it by averaging over its distribution.
\end{itemize}
\begin{center}
\vspace{-0.3cm}
