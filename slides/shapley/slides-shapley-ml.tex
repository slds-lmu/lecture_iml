...
\begin{frame}{Shapley Value - Definition \citebutton{Shapley (1953)}{https://doi.org/10.7249/P0295} \citebutton{Strumbelj et al. (2014)}{https://doi.org/10.1007/s10115-013-0679-x}}
  Using the order definition, the Shapley value for feature $j$ and a given observation $\xv$ is computed by:
     ...
     $$ \phi_j(\xv)  = \frac{1}{|P|!} \sum_{\tau \in \Pi} \underbrace{\fh_{\Sm \cup \{j\}}(\xv_{\Sm \cup \{j\}}) - \fh_{\Sm}(\xv_{\Sm})}_{\text{marginal contribution of feature $j$}} $$ %S \subseteq P\setminus \{j\}
...
\begin{itemize}
  ...
  Feature value $x_j$ contributed amount $\phi_j$ to prediction $\fh(\xv)$ compared to the average prediction
   ...
   %$$ \phi_j(\xv) = \frac{1}{|P|! \cdot n} \sum_{\tau \in \Pi} \sum_{i = 1}^n \fh_{\Sm \cup \{j\}}^{(i)}(\xv_{\Sm \cup \{j\}}) -  \fh_{\Sm }^{(i)}(\xv_{\Sm}) $$
    $$ \phi_j(\xv) = \frac{1}{|P|! \cdot n} \sum_{\tau \in \Pi} \sum_{i = 1}^n
   \fh(\xv_{\Sm \cup \{j\}}, \xv_{- \Sm \cup \{j\}}^{(i)}) - \fh(\xv_{\Sm}, \xv_{- \Sm}^{(i)})
   $$
...
%\vspace{0.25cm}
  \begin{enumerate}[<+->]
      ...
      \begin{enumerate}
        \item Select random permutation of feature indices $\tau = (\tau^{(1)}, \ldots, \tau^{(p)}) \in \Pi$
        %\item Select random data point $\xv^{(k)} \in X$.
        \item Select random data point $\mathbf{z}^{(m)} \in \D$%\\
        %$\leadsto$ used to marginalize over
        \item Let $S_m := \Sm$ be the set of features before $j$ of order $\tau$ forming a coalition
        ...
        \item Construct two observations by replacing feature values from $\xv$ with $\mathbf{z}^{(m)}$:
          \begin{itemize}
          \setlength\itemsep{.5em}
            \item $\xv_{+j}^{(m)} = (\underbrace{x_{\tau^{(1)}}, \ldots, x_{\tau^{(j-1)}}, x_{\tau^{(j)}}}_{\xv_{S_m \cup \{j\}}}, z_{\tau^{(j+1)}}^{(m)} , \ldots, z_{\tau^{(p)}}^{(m)} )$ (with value of feature $j$ from $\xv$)
            \item $\xv_{-j}^{(m)} = (\underbrace{x_{\tau^{(1)}}, \ldots, x_{\tau^{(j-1)}}}_{\xv_{S_m}}, z_{\tau^{(j)}}^{(m)} , z_{\tau^{(j+1)}}^{(m)} , \ldots, z_{\tau^{(p)}}^{(m)} )$ (without value of feature $j$ from $\xv$)
          \end{itemize}
        \item Compute difference $\phi_j^m = \fh(\xv_{+j}^{(m)}) - \fh(\xv_{-j}^{(m)})$ \\
        $\leadsto$ Random sample from the marginal contribution of feature $j$ for a sampled coalition
      \end{enumerate}
    \item Compute Shapley value $\phi_j = \frac{1}{M}\sum_{m=1}^M \phi_j^m = \frac{1}{M} \sum_{m = 1}^M
   \fh(\xv_{+j}^{(m)} ) -  \fh(\xv_{-j}^{(m)} ) $
  \end{enumerate}
%\pause
%Each $\phi_j^m$ is a random sample from the marginal contribution of feature $j$ for a sampled coalition introduced by $\tau$.
   
   %$$ \phi_j(\xv) = \frac{1}{|P|! \cdot n} \sum_{\tau \in \Pi} \sum_{i = 1}^n  \fh(\xv_{\Sm \cup \{j\}}, \xv_{- \Sm \cup \{j\}}^{(i)}) - \fh(\xv_{\Sm}, \xv_{- \Sm}^{(i)})  $$
...
\begin{frame}{Shapley value computation - Illustration}

\begin{exampleblock}{Definition}
% \[
% \tikzmark{phi}\phi_{\tikzmark{j}j}(%\tikzmark{v}\fh,
% \tikzmark{x}\xv)=\frac{1}{M}\tikzmark{M}\sum_{m=1}^M \left[
% \fh_{S_m \tikzmark{SJ}\cup j}
% \left(\xv_{S_m \tikzmark{SJ}\cup j}\right)-
% \fh_{\tikzmark{S}S_m}\left(\xv_{\tikzmark{S}S_m}\right)\right]
% \]
$$
\tikzmark{phi}\phi_{\tikzmark{j}j}(%\tikzmark{v}\fh,
\tikzmark{x}\xv)=\frac{1}{M}\tikzmark{M}\sum_{m=1}^M \left[
...
\node<2>[expl] 
  (Sex) 
  at (12,2cm)
  {$\xv$ with replaced feature values in $S_m$};
\node<3>[expl] 
  (Sex) 
  at (13,1cm)
 ...
 \node<2>[expl] 
  (SJex) 
  at (10,-0.8cm)
  {$\xv$ with replaced feature values in $S_m \cup \{j\}$};
  \draw<2>[arrow]
 ...
 \draw<3>[arrow]
  ...
  \draw<3> [decorate,decoration={brace, amplitude=5pt,mirror,raise=4ex}]  (7,1.1) --  (10.4,1.1)
  node[midway,yshift=-3.4em]{\scriptsize $:= \Delta(j, S_m)$};
  
  \draw<3> [draw=white, fill=white, opacity=0.6]
       (4,0) -- (7,0) -- (7, 2) -- (4,2) --cycle;
  %%%% Explain Formula III
  \draw<4> [draw=white, fill=white, opacity=0.6]
...
\begin{itemize}
    ...
    %\end{onlyenv}
    \begin{onlyenv}<3>
     \item $\Delta(j, S_m) = \fh(\xv_{+j}^{(m)}) -
\fh(\xv_{-j}^{(m)})$ is the marginal contribution of player $j$ to coalition $S_m$
...
\includegraphics<3>[page=5, width=1\textwidth]{figure_man/data_shapley}
\includegraphics<4>[page=6, width=1\textwidth]{figure_man/data_shapley}
\end{center}
