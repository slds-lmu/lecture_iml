...
\begin{vbframe}{Estimation Algorithm}
 DRAFT
Estimation algorithm for given model $\fh$ and observation $\xi$.
  \begin{enumerate}
      \item Set $\phi_j = 0$
      \item For 1 to m, \textbf{do}:
      \begin{enumerate}
        \item select at random, permutation $\pi \in \Pi$
        \item select at random, data point $x^{(k)} \in X$
        ...
        \item construct two instances:
          \begin{itemize}
            \item with feature j: TODO: Continue here
          \end{itemize}
      \end{enumerate}
  \end{enumerate}
\begin{center}
