...
\item Feature description (modified):
\begin{itemize}
...
\item \code{weekday}: day of the week (0: SUN, 1: MON, ..., 6: SAT)
\item \code{workingday}: day is not a weekend or holiday (0: NO WORKING DAY, 1: WORKING DAY)
\item \code{weathersit}: weather situation (0: GOOD, 1: MISTY, 2: RAIN/SNOW/STORM)
\item \code{temp}: temperature in Celsius
\item \code{hum}: humidity in percent
\item \code{windspeed}: wind speed in km/h
\item \code{days\_since\_2011}: Number of days since start of historical log (January 1st, 2011) to account for the trend over time
\end{itemize}
\end{itemize}
\end{frame}
