...
\item What is Explainable Artificial Intelligence (XAI) and how does it differ from IML?
...
\begin{itemize}
\item Machine learning (ML) algorithmically trains predictive models with no or little pre-specifications or assumptions.
...
\begin{itemize}
\item IML is often used synonymously with Explainable AI (XAI), as ML is often used synonymously with AI. There is no unified standard for these terminologies. We find that XAI often is specifically concerned with the interpretation of neural networks, whereas IML is used as an encompassing term for everything related to model interpretability, i.e., interpretable models such as generalized additive models, model-agnostic techniques, as well as interpretations of neural networks.
\item The nature of neural networks allows for powerful model-specific interpretation techniques, e.g., layer-wise relevance propagation (LRP) and saliency maps. They have in common that influence on the output layer is backpropagated layer by layer through the entire network up to the input layer.
