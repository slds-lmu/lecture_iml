...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    % \]
  \end{itemize}
  \item Select the split point \( s_k \) that minimizes \( \text{RSS}_\text{split}(s_k) \)
...
\begin{itemize}
  ...
  \item \textbf{Search:} Choose best split \(s_k^\star = \arg\max_{s_k} \Delta\text{RSS}(s_k)\)

  \item \textbf{Complexity per feature:}\; \(O(n\log n)\) (sorting) + \(O(n)\) (cumulative sums \& scan)
\end{itemize}
\end{frame}
