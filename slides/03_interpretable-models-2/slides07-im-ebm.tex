...
\begin{itemize}
  \item For each region $r \in \{A, B, C, D\}$, compute:
    ...
    \]
  \item Plug into RSS: $$
\text{RSS}(c_i, c_j) = \sum_{r} \left( \sum_{(x,y) \in r} y^2 - 2 \hat{y}_r \sum_{(x,y) \in r} y + \hat{y}_r^2 \cdot n_r \right) = 
%\sum_{r}\!\left(\sum_{(x,y)\in r} y^2-\frac{T_r^{2}}{n_r}\right)
\sum_{r}\sum_{(x,y)\in r} y^2+\sum_{r}\frac{T_r^{2}}{n_r}
$$
  %\item Requires only scalar summaries $(T_r, Q_r, n_r)$
...
\begin{align*}
  \Delta\text{RSS}(c_i,c_j)
     &=\text{RSS}_0-\text{RSS}(c_i,c_j)
     &=\left(\sum_{k}y_k^{2}-\frac{T^{2}}{n}\right)
       -  \sum_{r}\sum_{(x,y)\in r} y^2+\sum_{r}\frac{T_r^{2}}{n_r}
\end{align*}
\end{itemize}
...
Because $\sum_k y_k^{2} = \sum_{r}\sum_{(x,y)\in r} y^2$, \emph{all squared target terms cancel}:
\[
  \boxed{\displaystyle
  \Delta\text{RSS}(c_i,c_j)
     =\sum_{r}\frac{T_r^{2}}{n_r}\;-
      \frac{T^{2}}{n}}
\]
The parent term $T^{2}/n$ is constant across all cuts.  Hence
\[
  \textbf{maximize }S(c_i,c_j)=\sum_{r}\frac{T_r^{2}}{n_r}
...
\begin{itemize}
  ...
  \item Enables fast lookup of region statistics \(T_r, n_r\) for any cut
...
%   \item \textbf{Lookup stats}: obtain $(T_r,n_r)$ for the four rectangles
%         in $O(1)$s.
%   \item \textbf{Score}: compute $S(c_i,c_j)=\sum_{r}T_r^{2}/n_r$.
