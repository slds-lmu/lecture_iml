...
\begin{itemize}[<+->]
    \item \textbf{PFI Idea:} Replace feature of interest $X_j$ with an independend sample from its marginal distibution $\P(X_j)$, e.g., by randomly permuting its values %$X_j$
    \item \textbf{Problem:} Under dependent features, permutation leads to extrapolation
    \item \textbf{Conditional Feature Importance Idea:} Sample $X_j$ from the cond. dist. $\P(x_j|x_{-j})$, s.t. the joint dist. is preserved, i.e., $\P(x_j|x_{-j}) \P(x_{-j}) = \P(x_j, x_{-j})$
\end{itemize}
%\\
%\lz
%\\
...
\begin{itemize}
  ...
  \item Repeat permuting $X_S$ (e.g., $m$ times) and average difference of both errors: 
