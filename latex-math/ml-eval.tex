...
\newcommand{\GEfull}[1][\ntrain]{\GE(\inducer, \lamv, #1, \rho)} % GE full
\newcommand{\GEhholdout}{\GEh_{\Jtrain, \Jtest}(\inducer, \lamv, |\Jtrain|, \rho)} % GE hat holdout
\newcommand{\GEhholdouti}[1][i]{\GEh_{\Jtraini[#1], \Jtesti[#1]}(\inducer, \lamv, |\Jtraini[#1]|, \rho)} % GE hat holdout i-th set
...
\newcommand{\GEind}{GE_n\left(\inducer_{L, O}\right)} % GE of inducer
...
\newcommand{\FJtestftrain}{\F_{\Jtest,\inducer(\Dtrain, \lamv)}} % predscore mat idxvec Jtest and model f
\newcommand{\FJtestftraini}[1][i]{\F_{\Jtesti[#1],\inducer(\Dtraini[#1], \lamv)}}  % predscore mat i-th idxvec Jtest and model f
