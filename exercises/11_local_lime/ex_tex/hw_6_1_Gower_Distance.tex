...
Here $\widehat{R}_j$ stands for the range of this feature (the ``diameter'' of the feature):
$\widehat{R}_j := \max_i(x_j^{(i)}) - \min_i(x_j^{(i)}).$
Despite its popularity, the Gower distance has a major drawback. In the following exercises, you should identify this problem and construct a solution.

\begin{enumerate}[a)]
\item
Let us consider the following example that should illustrate the problem:
