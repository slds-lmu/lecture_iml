...
insurance ("Berufsunfähigkeitsversicherung") with an ML model based on the 
...
\begin{enumerate}[a)]
  ...
  Fill out the missing fields in the following table, where $\phi_{\sigma}(\zv)$ is the 
  exponential kernel similarity measure for a specific $\sigma$:
  $\phi_{\sigma}(\zv) = \exp(-d(\xv, \zv)^2/ \sigma^2)$.
...
library(StatMatch)
 ...
 dist = round(c(StatMatch::gower.dist(x, z))[1:3],2)
...
@
  ...
  Compute the local faithfulness for both surrogate models using the weights from a) for $\sigma = 0.15$. \\
