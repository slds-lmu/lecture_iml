In the following, you are guided to implement LIME to interpret a Support Vector Machine (SVM). We use two features and explore LIME on a multi-classification problem with only two (numeric) features. The associated files with templates and an example for this exercise are \textit{lime.py} or \textit{lime.R} depending on your preferred programming language. 
...
\begin{itemize}
  \item The function \texttt{get\_grid()} prepares data to visualize the feature space. It create a $N\times N$ grid, and every point in this grid is associated with a value. This value is obtained by the model's predict method.
  \item The function \texttt{plot\_grid()}, visualizes the prediction surface. 
  \item The created plot is an input to the function \texttt{plot\_points\_in\_grid()}, which adds committed data points to the plot. 
\end{itemize}
