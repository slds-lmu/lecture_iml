...
a given observation $x^\star$ (\texttt{x\_interest}) the closest data point which has a prediction equal to 
...
\begin{enumerate}[a)]
  ...
  \begin{enumerate}
    ...
    setting them to the value of \texttt{x\_interest} still leads to a different prediction than \texttt{x\_interest}.
  \end{enumerate}
  
  Try out your function given the code example.
  Which features could be reset to the value of \texttt{x\_interest}, which ones not?
