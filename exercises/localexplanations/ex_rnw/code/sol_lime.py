...
def plot_grid(u, v, z, labels, title=None, embedded=False):
    ...
    plt.pcolorgrid(u, v, z, cmap='viridis', shading='auto')
    plt.colorbar()
    plt.grid(alpha=0)
...
def weight_points(x_interest, X, kernel_width=0.2):
    ...
    Parameters:
        x_interest (np.ndarray): Single point with shape (2,).
        X (np.ndarray): Points with shape (?, 2).
        kernel_width (float): kernel_width value to calculate distance.
    ...
    weights = []
    for x in X:
        dist = np.exp(-np.linalg.norm(x_interest-x)/(kernel_width*kernel_width))
        weights.append(dist)
