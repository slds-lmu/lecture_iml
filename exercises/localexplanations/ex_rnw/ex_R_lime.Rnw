...
In these files, helper functions for plotting (\texttt{plot\_grid()} and \texttt{plot\_points\_in\_grid()}) were already implemented.
...
The function \texttt{plot\_grid()}, visualizes the prediction surface. The create plot is an input to the function \texttt{plot\_points\_in\_grid()}, which adds specified data points to the plot. 
