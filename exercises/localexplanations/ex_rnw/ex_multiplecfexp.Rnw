...
print(xtable::xtable(x = x), include.rownames = FALSE)
@
...
\begin{enumerate}[a)]
  ...
  \textit{Hint:} Think about the different properties that counterfactuals should fulfill. Note that there exist multiple strategies.
