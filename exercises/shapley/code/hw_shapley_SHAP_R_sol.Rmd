...
### Modelling a random forest using the ranger package. Other packages are possible  
  ### as well.
classifier_RF <- ranger(Man.of.the.Match ~ ., training_data, probability = TRUE)
classifier_RF

### Example instance
predict(classifier_RF, X_test[1:5,])$predictions[, 2]
...
marginal_vfunc <- function(S, observation, X, model, nr_samples = 100) {
  ...
  ### predictions.
  mean(predict(model, X_tmp)$predictions[, 2])
}
...
shapley_test <- function(population, shapley_fct, v_function=payoff) {
  ...
  print(isTRUE(all.equal(sum_, v_function(population), tolerance = 1e-8)))
}
...
shap_weights <- function(mask) {
  ...
  weights <- (p - 1) / (choose(p, z_prime_abs) * z_prime_abs * (p - z_prime_abs))
...
kernel_shap <- function(obs, X, n_samples, model) {
  ...
  pred <- predict(model, df)$predictions[, 2]
