...
\label{ex:Shapley_axioms_proofs}
...
Throughout this exercise, we use the fact that the order and the set definition of Shapley values yield the same result (which can be shown by a counting argument), so you can always choose which definition to use.
...
\begin{enumerate}
    ...
    \textit{Hint:} This is easier when using the order definition. Plug the definition into the sum inside the axiom, then swap the sums and argue why almost all terms of the inner sum vanish.
    
    \item \textbf{Bonus:} Similarly to the additivity axiom, prove the following homogeneity axiom:
    ...
    \textit{Note:}
    This axiom together with the additivity axiom shows that the Shapley value is linear in the payout function, so we have linearity of the Shapley value.
