...
where $t, m, s, j, l$ indicate whether Timnit, Margret, Samy, Jeff, and Larry are in the set $S$ (as Boolean variables, i.e.\ 1 if the player is in $S$, 0 otherwise), and $\wedge$ denotes logical AND, $\vee$ denotes logical OR. Let $v := \text{payoff}$ denote the value function of this game. The function \texttt{payoff(coalition)} is supposed to implement this functionality, where a set is a list/set of the respective characters (\texttt{'t', 'm', 's', 'j', 'l'}).
...
\begin{enumerate}
    ...
    \end{itemize}
    Test your functions with the examples from \ref{ex:shapley_a} or with other examples from this game, in particular corner cases like the empty set or the full set of all players.
    ...
    \begin{enumerate}
        ...
        \item \textit{Efficiency:} Player contributions add up to the total payout of the game:
            $$\sum\nolimits_{i=1}^p\phi_i = v(P),$$
            where $p = |P|$ is the number of players.
            
        \item \textit{Symmetry:} Assess whether two features with the same contributions have the same Shapley value. I.e., for any two players $\{j,k\}$, if for any $S \subseteq P\, \setminus \, \{j,k\}$ we have:
            $$v(S \cup \{j\}) - v(S) = v(S \cup \{k\}) -  v(S),$$
        then the Shapley values of $j$ and $k$ should be identical.
        
        \item \textit{Dummy:} Given a player $j$ which makes no contribution for all subsets $S \subseteq P \setminus \{j\}$ (that is $v(S) = v(S \cup \{j\})$), the Shapley value of this player must be zero.
