...
\begin{enumerate}[a)]
    ...
    Test your implementation on different feature subsets for the instance from \ref{ex:shap_a}.
    ...
    Calculate \texttt{shapley\_perm\_approx()} from the Shapley value implementations using \texttt{marginal\_vfunc()} and the random forest from \ref{ex:shap_a} on your test instance.
    ...
    \begin{itemize}
    	...
    	\item \texttt{replace\_dataset(obs, X, nr\_samples)}: A function to create the augmented dataset of \texttt{nr\_samples} many samples by (1) sampling coalitions (saved in \texttt{mask}) and (2) mapping them to the original feature space using background data from \texttt{X}.
        You can directly sample \texttt{nr\_samples} many data points as well as coalitions, so sampling a new random background sample for each coalition, you do not need to generate multiple background samples for every coalition or use the same background sample for different coalitions. Note that this is a simplified version; the standard algorithm averages over multiple background samples per coalition.
