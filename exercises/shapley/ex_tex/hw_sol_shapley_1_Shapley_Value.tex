...
\begin{enumerate}[a)]
	...
	\begin{algorithm}[h]
		...
		\begin{algorithmic}[1]
			...
			\State \textbf{return} 10 * \texttt{t} + 10 * \texttt{m} + 10 * \texttt{s} + 2 * \texttt{j} + 20 * (\texttt{t} and \texttt{m}) + 20 * (\texttt{t} and \texttt{m} and \texttt{s}) - 30 * ((\texttt{t} or \texttt{m} or \texttt{s}) and \texttt{j})
		\end{algorithmic}
	\end{algorithm}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\For{\texttt{i} = 1 to \texttt{num\_iter}}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\EndFor
			\If{\texttt{surplus\_all\_j} equal \texttt{surplus\_all\_k}}
			...
			\EndIf
			\State \textbf{print} \textit{unequal surplus, axiom trivially satisfied}
	...
	\begin{algorithm}[H]
		...
		\begin{algorithmic}[1]
			...
			\State save \texttt{surplus\_j} in vector \texttt{surplus\_all\_j} for every iteration
			\EndFor
			\If{sum of $|$\texttt{surplus\_all\_j}$|$ ==\, 0}
			\State \textbf{print} \textit{is dummy player}
			\State \texttt{val\_j} $\gets$ \texttt{shapley\_func(j, population, v\_function)}
