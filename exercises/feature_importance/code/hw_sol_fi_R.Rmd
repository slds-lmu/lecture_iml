...
If a feature is already independent of its covariates ($x_j \perp x_{-j}$), then PFI does not extrapolate to unseen regions.

Intuitively, the reason is that no dependency with the covariates is broken, since there were no dependencies between $x_j$ and the remaining variables $x_{-j}$ to begin with.
...
sample_cond_gaussian <- function(J, C, X_train, X_test, num_samples) {
  ...
  as.data.frame(sapply(1:length(mu_bar_all_C),
                       function(i) rnorm(num_samples, mu_bar_all_C[i], sqrt(sigma_bar))))
}
```
...
- $X_3$ is conditionally dependent on $Y$ ($X_3 \not \perp Y | X_1, X_2, X_4$) and it is used by the model
- without further assumptions, we cannot make conclusions about the remaining variables. We only know that they are not at the same time conditionally dependent on $Y$ and used by the model for its prediction.

# Exercise 3: Refitting based importance
## 3a)
```{r}
loco <- function(fname, original_model, X_test, y_test, X_train, y_train) {
  ### Refit a model without the feature of interest.
  loco_X_train <- X_train[ , colnames(X_train) != fname, drop = FALSE]
  loco_X_test <- X_test[ , colnames(X_test) != fname, drop = FALSE]

  ### Generate the formula and train the reduced OLS model.
  f <- as.formula(paste("y_train ~", paste(colnames(loco_X_train), collapse = " + ")))
  new_model <- lm(f, data = cbind(loco_X_train, y_train = y_train))
  ...
  loco_mse <- mean((y_test - predict_for_loco) ^ 2)
...
```{r}
### The exact same procedure to generate the plot as in exercise 1.
loco_results <- n_times(fi, 10, FALSE, loco, model, X_test, y_test, X_train, y_train)
