\loesung{	
	\begin{enumerate}[a)]
		...
		\item If a feature is already independent of its covariates ($x_j\perp x_{-j}$), then PFI does not extrapolate to unseen regions.

		Intuitively, the reason is that no dependence with the covariates is broken, since there were no dependencies between $x_j$ and the remaining variables $x_{-j}$ to begin with.
		...
		\begin{algorithm}[H]
			...
			\begin{algorithmic}[1]
				...
				\Require \texttt{predict}: prediction function
		...
		\begin{algorithm}[H]
			...
			\begin{algorithmic}[1]
				...
				\State \texttt{imp} $\gets$ \texttt{perf\_pert(fname, predict, score, X\_test, y\_test, ...)}
		...
		\begin{algorithm}[H]
			...
			\begin{algorithmic}[1]
				...
				\Require \texttt{return\_raw}: Whether only the aggregation (mean, std) or also the raw results are returned
				...
				\State \texttt{std\_fi} $\gets$ std of \texttt{results}
		...
		\begin{itemize}
			\item $X_1$, $X_2$, $X_3$ are used by the model for its prediction
