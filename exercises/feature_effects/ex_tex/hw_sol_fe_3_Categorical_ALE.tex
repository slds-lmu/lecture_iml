...
\item 
The pseudocode in Algorithm~\ref{ex_alg:ALE_cat_get_diff_cat} computes the pairwise sum of the absolute differences of relative 
frequencies in the categories of a categorical feature $x_j$ based on a 
feature $x_k$

%\lstinputlisting{"rsrc/get_diff_cat().R"}

\begin{algorithm}[ht]\label{ex_alg:ALE_cat_get_diff_cat}
