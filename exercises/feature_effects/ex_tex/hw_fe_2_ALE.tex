...
The wheat seeds dataset contains 210 observations of wheat kernels with 7 geometric features (area, perimeter, compactness, kernel length, kernel width, asymmetry coefficient, groove length). The target is the seed type.

Recall the ALE estimation formula from the lecture:
$$\hat{\tilde{f}}_{j,ALE}(x_j) = \sum_{k=1}^{k_j(x_j)} \frac{1}{n_j(k)} \sum_{i:\, x_j^{(i)} \in (z_{k-1},\, z_k]} \left[\hat{f}(z_k, \mathbf{x}_{-j}^{(i)}) - \hat{f}(z_{k-1}, \mathbf{x}_{-j}^{(i)})\right] - c$$
where $z_0 < z_1 < \ldots < z_K$ are the interval boundaries and $c$ is a centering constant.

\begin{enumerate}[a)]
  \item Since ALE works with bins, we have to select bounds first. Usually the bins are chosen s.t. every bin has roughly the same number of data points (quantile-based bins). However, to make it more simple, we use the same length for every interval (equidistant bins).

  \textit{Note:} In practice, quantile-based bins are preferred because they ensure a similar number of observations per bin. We use equidistant bins here for simplicity.

  Complete the function \texttt{get\_bounds()} to select bounds between the minimum and maximum value of a feature by using \texttt{n\_intervals}. Those bounds are used for the calculation of ALE in the next tasks.
...
\begin{itemize}
    \item Select the observations inside the interval (if zero observations are found then return 0 for this interval -- this is an approximation, since the true local effect in that region is unknown due to lack of data). In general $x_s$ can be counted as included if it is bigger than the lower bound and less than or equal to the upper bound. However, make sure the elements in the first bound are included as well.
...
Finally, if the \texttt{centered} parameter is set, center each interval by subtracting the average of all interval values.\\
