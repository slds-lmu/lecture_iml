...
\caption{PDP of personal\_status\_sex. The left panel shows the predicted probability of ``good'' credit risk (low risk), the right panel shows the predicted probability of ``bad'' credit risk (high risk) for each category.}
\label{fig:pdp}
\end{figure}
...
\begin{itemize}
  ...
  \textit{Hint:} In R, the command \texttt{expand.grid(unique(feature.j), unique(feature.j))} creates the first two columns in
your resulting dataset -- the pairwise class combinations for $x_s$. In Python, you can use \texttt{itertools.product(unique\_levels, repeat=2)} similarly.
