...
These boundaries define the intervals over which we estimate local effects.
...
\begin{algorithmic}[1]
\Require \texttt{model} : trained model
\Require \texttt{X}: input data 
...
\EndFor
\State \texttt{uncentered\_ale} $\gets$ cumulative sum of \texttt{results}
...
The cumulative sum reconstructs the main effect by integrating the local effects. Centering ensures the ALE has mean zero, so it represents deviations from the average prediction.
...
\Require \texttt{n\_intervals}: number of intervals 
\Require \texttt{centered}: whether to return centered or uncentered ALE
\State \texttt{bounds, y} $\gets$ \texttt{calculate\_ale}(\texttt{model}, \texttt{X}, \texttt{s}, \texttt{n\_intervals}, \texttt{centered})
\State \texttt{lowerbound} $\gets$ lower interval bounds
\State \texttt{upperbound} $\gets$ upper interval bounds
...
Plotting ALE values at interval centers gives us the feature effect curve, which shows how the feature influences predictions relative to the average.
