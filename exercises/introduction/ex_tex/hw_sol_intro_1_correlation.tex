...
Note: The variable $y$ given in the dataset is not needed for this exercise; we only work with the features $x_1$ and $x_2$.
...
Then:
  ...
  \item The Pearson correlation coefficient is a measure of \textit{linear} dependence only. A value of $\rho \approx 0$ does not imply independence -- it only means there is no linear relationship. As demonstrated by the scatter plot and the MI, strong non-linear dependencies can be completely invisible to Pearson correlation. When analyzing feature dependencies (e.g., for understanding an ML model), one should therefore not rely solely on linear correlation measures.
...
\bonusloesung{
  ...
  One possible solution: Estimate the distribution using histogram binning. This practically means: We split the ranges of the features into intervals (buckets) and calculate the empirical distributions w.r.t. these buckets:
  \begin{center}
  ...
  Note: The choice of the number of bins affects the MI estimate. With fewer bins, we lose resolution; with too many bins relative to the sample size, estimates become unreliable. With only 9 data points, 3 bins is a reasonable compromise.
...
Now we can calculate the MI for this approximate distribution, which is very close to the MI of our original distribution: 
   ...
   \end{align*}
$\Rightarrow$
Since we used the natural logarithm, the MI is measured in \textit{nats}. The MI is always non-negative: for independent variables, $MI = 0$, and larger values indicate stronger dependency.
For our discretized variables with 3 bins each, the maximum possible MI is $\ln(3) \approx 1.099$ nats. Our value of approximately $0.603$ nats is thus about 55\% of the theoretical maximum, indicating a clear and strong dependency between $x_1$ and $x_2$ -- confirming what we observed in the scatter plot.
