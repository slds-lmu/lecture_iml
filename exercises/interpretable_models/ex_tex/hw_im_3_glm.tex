...
\begin{enumerate}[a)]
    ...
    \textit{The following parts d) and e) build on a different model specification using numerical features instead of season.}

    \item\label{ex:logreg_GLM_more_features}
    Next, we fit a complementary GLM model which considers the other four features except season, i.e., a model that is somewhat ``orthogonal'' to the first one in the sense that it uses a disjoint set of features.
    ...
    To visualize how temperature alone affects the predicted probability, we fix all other features at their mean values. This simplifies the linear predictor to a function of temperature only.
