...
\begin{enumerate}[a)]
  ...
  \item The following plot displays the derived PDP of b) and the ICE values of $X_1$. 
  The coloring reflects if the derived ICE value originates from an observation with 
  $x_2 = 1$ or $x_2 = 0$.
  Derive the conditional expectation functions for group $X_2 = 1$ and for group $X_2 = 0$ using Equation~\ref{eq:linmod} given the estimated coefficient values of b). 
