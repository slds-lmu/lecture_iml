...
In this exercise, we use the German Credit dataset and a random forest 
...
@
\caption{PDP of \texttt{personal\_status\_sex}}
\label{fig:pdp}
\end{figure}
...
In order to derive an artificial order, Molnar 2022 (Chapter 8.2)\footnote{https://christophm.github.io/interpretable-ml-book/ale.html} proposes to order the categories of the features $x_s$ according to their similarity based on other features in $x_c \in X_C$. In detail, the approach has the following steps: 
