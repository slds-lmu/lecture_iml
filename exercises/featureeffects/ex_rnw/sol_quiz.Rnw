\begin{enumerate}
  \item Which of the following statement(s) is/are correct?  
        \begin{enumerate}
        \item Interpretation methods are mainly needed to better explain real world phenomena. $\Rightarrow$ \textbf{No}
        \item Model-agnostic methods need access to gradients to explain a model. $\Rightarrow$ \textbf{No}
...
\item Which of the following statement(s) apply to feature effect methods? 
        ...
        \item The PDP of a feature provides information about possible interaction effects of the feature. $\Rightarrow$ \textbf{No}
        ...
        \item If features are uncorrelated, ALE plots are equal to PDPs. $\Rightarrow$ \textbf{No}
