...
ALE and PD plots are very similar. Increasing level of alcohol has a positive influence on the wine rating, while increasing level of acid.volatility has a negative effect on the predicted outcome. These two features show the highest volatility of global feature effects and thus can be considered most important for the model's prediction.
...
Note that the **iml** package plots the effects instead of the coefficients of the local model, i.e. $x_{ij} \beta_j$ instead of $\beta_j$.
...
```{r shapley, fig.width=10, fig.height=5}
idx1 = which(wine$quality == 8 & wine$type == "white")[11]
idx2 = which(wine$quality == 5 & wine$type == "red")[1]
shapley1 = Shapley$new(rfpred, x.interest = wine[idx1,])
shapley2 = Shapley$new(rfpred, x.interest = wine[idx2,])

library(patchwork)
plot(shapley1) + plot(shapley2)
```

The plot shows how much the actual prediction of the wine deviates from the average wine quality and how this deviation is attributed to the different features. In this specific example, we can see that the low acidity contributes most to the good rating of the white wine (left), while the low alcohol does not have a negative effect. This wine is one of the few great wines with low alcohol (see ICE plots). In contrast, for the not-so-good wine (right), the low alcohol does have a strong negative effect. 
...
Calculate global Shapley importance by the mean of absolute Shapley values over all sampled observations.
```{r shapley_shap_imp}
imp = shapley_values_all %>% dplyr::group_by(feature) %>% dplyr::summarise(imp = mean(abs(phi)))
imp[order(imp$imp, decreasing = TRUE),]

```
The ranking looks similar to PFI.

Based on the sample we can approximate the global behavior of features, e.g., by plotting the Shapley dependency for individual features
