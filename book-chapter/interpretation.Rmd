...
Specifically, we introduce methods implemented in the `r ref_pkg("iml")`, `r ref_pkg("DALEX")`, and `r ref_pkg("counterfactuals")` packages and most of them are also described in more detail in the introductory book about interpretable machine learning by @Molnar2022.
...
## Estimating Interpretation Methods on Train or Test Data {#sec-dataset}
...
For example, these methods are useful to identify whether the model estimated a non-linear relationship between a feature of interest and the target outcome.
...
two_features = cfe_multi$evaluate("no_changed", show_diff = TRUE)[no_changed == 3,]
```
...
cfe_multi$plot_surface(feature_names = c("bill_depth", "flipper_length"))
```
